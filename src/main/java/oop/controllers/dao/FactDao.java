...
public class FactDao extends Dao<Fact<? extends Entity, ? extends Entity>> {
    ...
    @Override
    public void insert(List<? extends Fact<? extends Entity, ? extends Entity>> values) throws RuntimeException {
        ...
        for (Method method : fact.getClass().getMethods()) {
            ...
            try {
                if ("getSubject".equals(methodName)) {
                    Entity subject = (Entity) method.invoke(fact);
                    if (subject != null)
                        propertiesMap.put("SubjectId", subject.getId());
                } else if ("getObject".equals(methodName)) {
                    Entity object = (Entity) method.invoke(fact);
                    if (object != null)
                        propertiesMap.put("ObjectId", object.getId());
                } else {
                    Object property = method.invoke(fact);
                    if (property != null)
